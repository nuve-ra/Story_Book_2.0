...
function CreateStory() {
  ...
  const generateStory = async () => {
    ...
    Please provide the story in the following JSON format:
    {
      "story_cover": {
        "title": "Story Title",
        "image_prompt": "Description for the cover image"
      },
      "chapters": [
        {
          "chapter_number": 1,
          ...
          "image_prompt": "Description for chapter 1 image"
        },
    ...
    try {
      ...
      if (!story.story_cover || !story.chapters || !Array.isArray(story.chapters)) {
  ...
  const SaveInDB = async (output: string) => {
    try {
      ...
      // Ensure formData includes imageStyle
      const completeFormData = {
        ...formData,
        imageStyle: formData.imageStyle || 'colorful' // Default to 'colorful' if not specified
      };

      const result = await saveStoryToDB(completeFormData, output, userEmail);
      return result;
    } catch (error) {
